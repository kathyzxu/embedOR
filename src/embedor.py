...
class EmbedOR:
    def __init__(self, nng_params={'mode': 'nbrs', 'n_neighbors': 15}, 
                 dim=2, p=3, epochs=300, perplexity=150, verbose=False, seed=10,
                 edge_weight='orc', subsample=False, subsample_factor=0.05, 
                 n_landmarks=None, landmark_selection='random', approx_affinities=False):
        ...
        self.X = None
        self.fitted = False
        self._timings = {}  # store method timings
    ...
    def print_timing_summary(self):
        ...
        logger.info("="*60)
        logger.info("EMBEDOR TIMING SUMMARY")
        logger.info("="*60)
        for method, t in sorted(self._timings.items(), key=lambda x: x[1], reverse=True):
            pct = (t/total)*100 if total>0 else 0
            logger.info(f"{method:30} | {t:8.4f}s | {pct:6.2f}%")
        logger.info("-"*60)
        logger.info(f"{'TOTAL':30} | {total:8.4f}s | 100.00%")
        logger.info("="*60)
    ...
    def fit_transform(self, X=None):
        ...
        self._layout(affinities=self.all_affinities, repulsions=self.all_repulsions)
    ...
    def _build_nnG(self):
        if self.X is None:
            raise ValueError("Data must be provided to build nearest neighbor graph.")
        return_dict = get_nn_graph(self.X, self.nng_params)
        self.G = return_dict['G']
        logger.info(f"Nearest Neighbor Graph built with {len(self.G.nodes())} nodes and {len(self.G.edges())} edges.")
    ...
    def _compute_curvatures(self):
        if self.edge_weight == "orc":
            ...
            self.G = result['G']
        logger.info(f"Computed {self.edge_weight.upper()} for each edge.")
    ...
    def _compute_distances(self, max_val=np.inf):
        ...
        if self.edge_weight != "euclidean":
            c = 1 / np.log((self.k_max - self.k_min) / (self.k_crit - self.k_min))
            for idx, (u, v) in enumerate(self.G.edges()):
                orc = self.curvatures[idx]
                energy = (-c * np.log(orc - self.k_min) + c * np.log(self.k_crit - self.k_min) + 1)**self.p + 1
        ...
        logger.info("Computed edge energies and APSP.")
    ...
    def _compute_affinities(self):
        ...
        if self.approx_affinities:
            ...
            self.all_affinities = squareform(joint_probabilities_nn(apsp_perp, desired_perplexity=self.perplexity, verbose=self.verbose))
        else:
            self.all_affinities = squareform(joint_probabilities(self.apsp, desired_perplexity=self.perplexity, verbose=self.verbose))
        ...
        logger.info("Computed affinities.")

    @timer
    def _init_embedding(self):
        A_affinity_sparse = nx.to_scipy_sparse_array(self.G, weight='affinity', nodelist=list(range(len(self.G.nodes()))))
        self.spectral_init = spectral_layout(data=None, graph=A_affinity_sparse, dim=self.dim, random_state=self.seed)
        ...
        logger.info("Initialized spectral embedding.")
    ...
    def _subsample_interactions(self):
        ...
        logger.info(f"Subsampled {self.subsample_indices.shape[1]} interactions.")
    ...
    def _update_G(self):
        ...
        logger.info("Updated graph with affinities and distances.")
