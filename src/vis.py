# from https://github.com/pachterlab/GFCP_2022/blob/main/vis.py
...
emb_dict={"default":'pcs',"PCA":"ps", "UMAP":"us", "tSNE":"ts", "PHATE":"phate", "EmbedOR":"embedor"}
...
def makeEmbeds(vlm, embeds, x_name="S_norm", new_pca=True):
    ...
    if 'PHATE' in embeds:
        vlm.phate = phate.PHATE(n_components=2, n_jobs=8).fit_transform(vlm.pcs[:, :25])

    if 'EmbedOR' in embeds:
        vlm.embedor = EmbedOR().fit_transform(vlm.pcs[:, :25])
...
def plotGrid(ax,vlm,gridx,gridv,trans, scale=0.005, c="gray",alpha=1):
    '''
    ...
    ax.quiver(x[masks, 0], x[masks, 1], v[masks, 0], v[masks, 1], width=0.005, scale=scale, scale_units='width', label=trans, color=c,alpha=alpha)
    return
